...
/// Runs slow work away from the event loop and hands its result back without
/// publishing the whole application. Callers use this when they can publish a
/// much smaller, purpose-built surface (the monitor preview is the important
/// case); forcing every background completion through [`Studio::publish`]
/// would rebuild the timeline for work that changed no timeline model.
