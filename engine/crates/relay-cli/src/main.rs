//! Command line driver for the Relay engine.
//!
...
#[derive(Parser)]
#[command(name = "relay", version, about = "Relay engine command line")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn probe(path: &PathBuf) -> Result<(), Box<dyn Error>> {
    let info = relay_media::probe(path)?;
...
fn render(
    ...
    let info = relay_media::probe(input)?;
