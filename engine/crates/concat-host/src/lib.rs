...
//!
//! It used to be the Tauri host of the React editor, where every one of
//! these was a `#[tauri::command]` converting wire formats. There is no wire
//! any more - the Slint window calls these functions in-process - so what is
//! left is the plumbing itself, with the same doctrine: no editing decision
//! is made here. If a function starts deciding what an edit means, it
//! belongs in `concat-project`.
//!
