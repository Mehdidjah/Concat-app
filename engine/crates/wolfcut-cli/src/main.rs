//! Command line driver for the WolfCut engine.
//!
...
use wolfcut_core::timeline::{Clip, MediaRef, Timeline, Track, TrackKind};
use wolfcut_media::{EncodeOptions, FfmpegEncoder, FrameSink, ReaderPool};
use wolfcut_render::{Compositor, CpuCompositor, Layer, plan_frame};

#[derive(Parser)]
#[command(name = "wolfcut", version, about = "WolfCut engine command line")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn probe(path: &PathBuf) -> Result<(), Box<dyn Error>> {
    let info = wolfcut_media::probe(path)?;
...
fn render(
    ...
    let info = wolfcut_media::probe(input)?;
