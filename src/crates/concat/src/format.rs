...
mod tests {
    ...
    fn zooming_in_reveals_the_fine_buckets_and_never_loses_a_peak() {
        ...
        let coarse_spikes: Vec<_> = coarse
            .split(" Z ")
            .enumerate()
            .filter_map(|(index, bar)| bar.contains(" 0.0000 L").then_some(index))
            .collect();
        assert!(
            (1..=2).contains(&coarse_spikes.len()),
            "the spike survives the fold in at most two adjacent columns: {coarse}"
        );
        assert!(
            coarse_spikes.windows(2).all(|pair| pair[1] == pair[0] + 1),
            "a reduced peak bucket cannot light up separated columns: {coarse}"
        );
